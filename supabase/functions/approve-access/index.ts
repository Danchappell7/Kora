// ============================================================
// KANBO — approve an early-access request + email the person (Deno / Supabase)
// Admin-only. Marks the request approved, flips any matching profile to
// approved, and sends a "you're in" email via Resend with a link to sign in.
//
...
Deno.serve(async (req) => {
  ...
  // identify + authorise the caller (must be the admin)
  ...
  try {
    ...
    // approve the request + flip any existing profile with that email
    ...
    // email them (best-effort — approval succeeds even if mail isn't configured)
    let emailed = false;
    ...
    if (resendKey) {
      ...
      const r = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({ from, to: reqRow.email, subject: "You’re in — your Kanbo access is approved", html }),
      });
      emailed = r.ok;
