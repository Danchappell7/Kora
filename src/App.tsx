...
function TasksPage({ tasks, allTasks, projects = [], view, setView, groupBy, setGroupBy, smart, setSmart, onOpen, onToggle, onToggleSubtask, onAdd, onMove, onBulkPatch, onBulkDelete, onPatch, onQuickAdd, onOpenImport, members, allTags, archivedTasks = [], header, sections = [], onCreateSection, onRenameSection, onDeleteSection, customFields = [], sectionField = "sectionId", sectionProjectId }: {
  tasks: Task[];
  allTasks: Task[];
  projects?: Project[];
  view: TaskView;
...
export default function App() {
  ...
  const renderMain = () => {
    switch (route.view) {
      ...
      case "search": return <SearchView tasks={tasks} projects={projects} members={assignees} currentUserId={currentUserId} onOpen={setDetailId} savedSearches={savedSearches} onSaveSearch={saveSearch} onDeleteSavedSearch={removeSavedSearch} preset={searchPreset} presetKey={route.list} onBulkPatch={bulkPatch} onBulkDelete={bulkDelete} />;
      ...
      case "project":
        return <TasksPage tasks={scoped} allTasks={allTasks} projects={wsProjects} view={view} setView={setView} groupBy={groupBy} setGroupBy={setGroupBy} smart={smart} setSmart={setSmart} onOpen={setDetailId} onToggle={toggleTask} onToggleSubtask={toggleSubtask} onAdd={openNewTask} onMove={(id, status, position) => {
          ...
          patchTask(id, patch);
        }} onBulkPatch={bulkPatch} onBulkDelete={bulkDelete} onPatch={patchTask} onQuickAdd={quickAddTask} onOpenImport={() => setImportOpen(true)} members={assignees} allTags={tags}
  ...
  return (
    <div style={{ position: "relative", height: "100vh", display: "flex", overflow: "hidden" }}>
      ...
      <ImportTasksModal open={importOpen} onClose={() => setImportOpen(false)}
        ...
        onImport={(rows) => { rows.forEach((r) => quickAddTask(r)); toastSuccess(`Imported ${rows.length} task${rows.length === 1 ? "" : "s"}`); }} />
