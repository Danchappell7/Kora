...
export function FocusMode({ focus, tasks, onClose, onOpenTask }: {
  ...
  const { running, seconds, setRunning, reset, targetMin, setTargetMin, taskId, setTaskId } = focus;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 120, background: "var(--bg-deep)", display: "flex", flexDirection: "column", animation: "fadeIn .35s var(--ease)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "20px 24px", position: "relative", zIndex: 2 }}>
        ...
        <button className="btn btn-ghost" onClick={onClose} style={{ marginLeft: "auto" }}><Icon name="x" size={16} /> Exit</button>
      </div>
        ...
        <div style={{ position: "relative", width: size, height: size }}>
          <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
            ...
            <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--accent)" strokeWidth={stroke} strokeLinecap="round"
              strokeDasharray={c} strokeDashoffset={c * (1 - pct / 100)} style={{ transition: "stroke-dashoffset 1s linear", filter: "drop-shadow(0 0 10px var(--accent))" }} />
          </svg>
          <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center" }}>
            <div style={{ textAlign: "center" }}>
              ...
              <div className="kicker" style={{ marginTop: 10, color: running ? "var(--accent)" : "var(--ink-4)" }}>{running ? "In flow" : "Ready"} · {targetMin}m goal</div>
            </div>
          </div>
        </div>
